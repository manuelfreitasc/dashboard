...
import { Room, Video, SyncState as RoomSyncState, RoomUserJoinedPayload, RoomUserLeftPayload } from '@/types';
...
const RoomPage = () => {
  ...
  // Socket store
  const { socket, connect: connectSocket, disconnect: disconnectSocket, emit, on, off, isConnected } = useSocketStore();

  // Component State
  const [roomDetails, setRoomDetails] = useState<Room | null>(null);
  const [videosInRoom, setVideosInRoom] = useState<Video[]>([]);
  const [currentSyncState, setCurrentSyncState] = useState<RoomSyncState | null>(null);
  
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Derived state for VideoPlayer
  const [currentPlayerState, setCurrentPlayerState] = useState<{src?: string; isPlaying: boolean; currentTime: number; title?: string}>({
    isPlaying: false,
    currentTime: 0,
  });
  ...
  useEffect(() => {
    ...
    return () => {
      if (socket) {
        console.log("Leaving room and disconnecting socket on component unmount");
        emit('room:leave', { roomId });
        // Consider if disconnect is always desired or if connection should persist across app
        // disconnectSocket(); 
      }
    };
  }, [authToken, connectSocket, roomId]); // Removed socket, isConnected, emit, fetchRoomData, router from dep array to simplify
  ...
  useEffect(() => {
    ...
    const handleSyncUpdate = (data: RoomSyncState) => {
      ...
      if (data.roomId === roomId) {
        ...
        toast.success(`Sync update received: ${data.isPlaying ? 'Playing' : 'Paused'} at ${data.progress.toFixed(0)}s for video ${data.title || data.currentVideoId}`);
      }
    };
    ...
    const handleNewChatMessage = (newMessage: ChatMessage) => {
      ...
      if (newMessage.userId !== currentUserId) { // Don't toast own messages if optimistically updated
  ...
  }, [socket, isConnected, roomId, emit, on, off, currentSyncState, currentUserId]); // currentSyncState & currentUserId added
  ...
  useEffect(() => {
    if (currentSyncState) {
      ...
      if (chatMessages.some(msg => msg.isOptimistic) && chatMessages.find(msg => msg.messageId === `${currentUserId}-${currentSyncState.lastEventTimestamp}` )) { // Example ID check
        // setChatMessages(prev => prev.filter(msg => !msg.isOptimistic));
      }

      const video = videosInRoom.find(v => v.id === currentSyncState.currentVideoId);
      ...
      console.log("Player state updated from sync:", {src: video?.url, isPlaying: currentSyncState.isPlaying, currentTime: currentSyncState.progress, title: video?.title});
    } else {
  ...
  // --- Render Logic ---
  if (isLoading && !roomDetails) return <div className="container mx-auto p-4 text-center">Loading room...</div>;
  if (error) return <div className="container mx-auto p-4 text-center text-red-500">Error: {error}</div>;
  if (!roomDetails) return <div className="container mx-auto p-4 text-center">Room not found.</div>;
  ...
  const handleSendMessage = (messageText: string) => {
    ...
    if (!currentUserId) { // Should have currentUserId if authenticated
    ...
    const optimisticMessage: ChatMessage = {
      ...
      username: roomDetails?.participants?.find(p => p.user.id === currentUserId)?.user.username || "You", // Attempt to find username
      timestamp: new Date(),
      isOptimistic: true,
    };
        ...
        <Card className="mb-4">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <Skeleton className="h-4 w-64" /> {/* Participants/Socket Status */}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-grow min-h-0">
          <div className="lg:col-span-2 flex flex-col gap-4 min-h-0">
            <Card className="flex-shrink-0">
              ...
              <CardContent>
                <Skeleton className="w-full rounded-md aspect-video bg-slate-900" /> {/* Video Player */}
              </CardContent>
              <CardFooter>
            ...
            <Card className="flex-grow flex flex-col min-h-0">
              <CardHeader><Skeleton className="h-6 w-1/3" /></CardHeader> {/* Playlist Title */}
              <CardContent className="flex-grow overflow-hidden flex flex-col">
                ...
                <div className="space-y-2 flex-shrink-0 pt-2 border-t">
                  <Skeleton className="h-9 w-full" /> {/* Add Video Title Input */}
                  <Skeleton className="h-9 w-full" /> {/* Add Video URL Input */}
  ...
  return (
    <div className="container mx-auto p-4 max-h-screen flex flex-col">
      <Card className="mb-4">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Room: {roomDetails?.name || <Skeleton className="h-6 w-32 inline-block" />}</CardTitle>
            <CardDescription>
              Participants: {roomDetails?._count?.participants ?? <Skeleton className="h-4 w-4 inline-block" />} | Socket: {isConnected ? 'Connected' : 'Disconnected'}
            </CardDescription>
          </div>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-grow min-h-0"> {/* min-h-0 is important for flex-grow in a flex col parent */}
        {/* Video Player and Playlist Section */}
        <div className="lg:col-span-2 flex flex-col gap-4 min-h-0">
          <Card className="flex-shrink-0"> {/* Video player card should not grow excessively */}
            <CardHeader>
              <CardTitle className="truncate">{currentPlayerState.title || "No video selected"}</CardTitle>
            </CardHeader>
            <CardContent>
              {currentPlayerState.src ? (
                <VideoPlayer
                  ...
                  onPause={handlePause}
                  onSeek={handleSeek}
                  onLoadedData={() => console.log("Video data loaded for src:", currentPlayerState.src)}
            ...
            <CardFooter>
              <p className="text-sm text-muted-foreground truncate">
                Playing: {currentPlayerState.title || "N/A"} | {currentPlayerState.isPlaying ? "Playing" : "Paused"} | {currentPlayerState.currentTime.toFixed(1)}s
              </p>
            </CardFooter>
          </Card>
          ...
          <Card className="flex-grow flex flex-col min-h-0">
            <CardHeader><CardTitle>Video Playlist</CardTitle></CardHeader>
            <CardContent className="flex-grow overflow-hidden flex flex-col"> {/* Allow content to take space and scroll */}
              <ScrollArea className="flex-grow pr-3 mb-3"> {/* ScrollArea takes available space */}
                {videosInRoom.length === 0 && <p className="text-sm text-slate-500">No videos yet.</p>}
                <ul className="space-y-1">
                  {videosInRoom.map((video) => (
                    <li key={video.id}
                        ...
                        title={video.title}>
                      {video.title}
                      <span className="text-xs text-muted-foreground block">By: {video.addedBy?.username || 'Unknown'}</span>
                    </li>
                  ))}
                </ul>
              </ScrollArea>
        ...
        <div className="lg:col-span-1 flex flex-col min-h-0"> {/* min-h-0 crucial for flex children with overflow */}
          <Card className="flex-grow flex flex-col"> {/* flex-grow allows card to fill space, flex-col for internal layout */}
            <CardHeader className="flex-shrink-0">
              <CardTitle>Live Chat</CardTitle>
            </CardHeader>
            <CardContent className="flex-grow overflow-hidden"> {/* This content area will allow ChatBox to be scrollable */}
              <ChatBox
